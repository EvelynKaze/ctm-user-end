// Client-compatible deposit API helpers

const apiUrl = process.env.API_URL || "https://ctm-backend-production.up.railway.app/api/v1";

export const createDeposit = async (
  data: {
    token_name: string;
    amount: number;
    token_deposit_address: string;
    user?: string | null;
    status?: string;
  },
  authToken?: string | null
) => {
  try {
    const depositPayload = {
      token_name: data.token_name,
      amount: data.amount,
      token_deposit_address: data.token_deposit_address,
      status: data.status || "pending",
    };

    const headers: Record<string, string> = { "Content-Type": "application/json" };
    if (authToken) headers["Authorization"] = `Bearer ${authToken}`;

    const response = await fetch(`${apiUrl}/deposits`, {
      method: "POST",
      headers,
      body: JSON.stringify(depositPayload),
      cache: "no-store",
    });

    const result = await response.json().catch(() => ({}));

    if (!response.ok) {
      console.error(`Failed to create deposit: ${response.status}`, result);
      return {
        success: false,
        message: result?.message || `Failed to create deposit (${response.status})`,
        code: result?.code,
      };
    }

    return result;
  } catch (error) {
    console.error("Create Deposit API Error:", error);
    return {
      success: false,
      message: error instanceof Error ? error.message : "Failed to create deposit",
    };
  }
};
